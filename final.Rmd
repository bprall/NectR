...
``` {r intro graph, echo=FALSE, warning=FALSE, message=FALSE}
...
# Graph of Colony Losses by State REORDER
...
#Boxplot that includes all the years showing colony loss by state 
#Conversation in relation to significance. 
...
#Save dredge output 
dredge(model4)
get.models() # Arrange by AIC weight 
Anova("best model")
```

```{r}
full_model <- lm(colony_lost_pct ~ state * month * stress_pct * stressor, data = selected_columns, na.action = "na.fail")

# Stepwise variable selection using AIC
stepwise_model <- step(full_model, direction = "both")

anova(stepwise_model)
dredge(stepwise_model)
...
```{r}
# Assuming 'data' is your dataset with columns: colony_lost_pct, state, month, stress_pct, stressor
install.packages("glmnet")
library(glmnet)

# Prepare data
X <- model.matrix(colony_lost_pct ~ state + month + stress_pct + stressor, data = selected_columns)
y <- selected_columns$colony_lost_pct
...
# Extract coefficients from the model using the optimal lambda
...
```
```{r}
Final_Model <- lm(formula =  colony_lost_pct ~ state + month + stress_pct + stressor + state:month + state:stress_pct + month:stress_pct + state:stressor + 
stress_pct:stressor + state:stress_pct:stressor, data = selected_columns)
