...
merged_data <- left_join(new_colony_data, new_stressor_data, by = c("year", "month", "state"))
```

``` {r graphs}
# Graph of Colony Losses by State (excluding "United States")
ggplot(filter(merged_data, state != "United States"), aes(x = state, y = colony_lost, fill = state)) +
...
# Graph of Colony Losses vs. Stress Percentage (excluding "United States")
ggplot(filter(merged_data, state != "United States"), aes(x = colony_lost, y = stress_pct)) +
  geom_point() +
  labs(title = "Colony Losses vs. Stress Percentage",
       x = "Colony Lost",
       y = "Stress Percentage")
```
